      ...
      <div className="rounded-lg border border-line bg-white/[0.03] p-4">
        ...
        <div className="mt-3 grid gap-2">
          {explanation.improvements.map((item, index) => (
            <p key={`improvement-${index}-${item}`}>{item}</p>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {(explanation.blockers.length > 0 ? explanation.blockers : ["No hard blocker from current checks."]).map((item, index) => (
            <span key={`blocker-${index}-${item}`} className="rounded-md border border-line bg-white/[0.04] px-3 py-2 text-xs text-slate-300">
              {item}
            </span>
          ))}
        </div>
